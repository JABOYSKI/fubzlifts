...
## V2 — Polish Branch
This is **fubzlifts v2.0**, a UX polish pass on top of the working v1.1 app.
- v1.1 (live, in-use at the gym): `g:\Vibecode\fubzlifts-v1.1` → deployed at `jaboyski.github.io/fubzlifts`
- v2 (this folder): not yet deployed. Iterate freely; do not deploy until polished.
- v2 uses the **same Supabase project** as v1.1 (schema unchanged), so testing happens against real data.
- **Chrome DevTools MCP** is configured at project scope (`.mcp.json`) — use it to inspect the running app in a real browser when working on UX/visual issues. Don't guess at layout — open the page and look.
- Goal of v2: smoother, more polished UX. Not a rewrite. Preserve all v1 features and behavior unless explicitly changing them.
